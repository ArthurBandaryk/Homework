    ...
    std::vector<Vertex>& v_vertexes, std::vector<short>& v_indexes)
{
    ...
    for (size_t i = 0; i < num_indexes; i += 3)
    {
        ...
        for (const auto& el : v_vertexes_interp)
        {
            ...
            if (pos.y < 0)
                pos.y = 0;
            if (pos.x >= static_cast<int32_t>(canv::width))
                pos.x = static_cast<int32_t>(canv::width - 1);
            if (pos.y >= static_cast<int32_t>(canv::height))
                pos.y = static_cast<int32_t>(canv::height - 1);
