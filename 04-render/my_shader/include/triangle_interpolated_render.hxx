...
struct Vertex {
  ...
  double u = 0.;        // texture pos x
  double v = 0.;        // texture pos y
  u_int8_t r = 0;       // r
  u_int8_t g = 0;       // g
  u_int8_t b = 0;       // b
  u_int8_t reserve = 0; //?
...
struct Uniform {
  ...
  double u = 0.;        // texture pos x
  double v = 0.;        // texture pos y
  double time = 0.;     // time for rotation figure
  u_int8_t r = 0;       // r
  u_int8_t g = 0;       // g
  u_int8_t b = 0;       // b
  u_int8_t reserve = 0; //?
  canv::Image *img = nullptr;
};
...
private:
  ...
  void raster_one_vertical_line(const Vertex &up, const Vertex &down,
...
public:
  ...
  void draw_raster_line(std::vector<Vertex> &v_vertexes,
                        std::vector<short> &v_indexes);
};
